...
def calculate_demographic_data(print_data=True):
    ...
    country_earning = df[['native-country', 'salary']].groupby('native-country').apply(lambda x: (x['salary'] == '>50K').sum() / x['salary'].count() * 100)
    highest_earning_country = country_earning.idxmax()
    highest_earning_country_percentage = country_earning.max().round(1)
